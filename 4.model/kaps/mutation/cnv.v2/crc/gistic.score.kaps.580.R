...
#####
tumor.seg.cnv <- read.table("4.model/kaps/mutation/cnv.v2/crc/gdac.broadinstitute.org_COADREAD-TP.CopyNumber_Gistic2.Level_4.2016012800.0.0/focal_input.seg.txt", sep="\t", header=T, stringsAsFactors=F)
tail(tumor.seg.cnv)
tumor.seg.cnv$Sample <- gsub("-[0-9A-Z]*-[0-9A-Z]*-[0-9A-Z]*-01$", "", tumor.seg.cnv$Sample)
tail(tumor.seg.cnv)
### subset segment file based on kaps group
seg.kaps=kaps.td
rownames(seg.kaps)=substr(rownames(seg.kaps),1,12)
head(seg.kaps)
length(intersect(rownames(seg.kaps), tumor.seg.cnv$Sample))
#
# Get subtype segments information
set1.seg.cnv <- tumor.seg.cnv[tumor.seg.cnv$Sample %in% rownames(subset(seg.kaps, kaps.group=="set1")),]
set2.seg.cnv <- tumor.seg.cnv[tumor.seg.cnv$Sample %in% rownames(subset(seg.kaps, kaps.group=="set2")),]
set3.seg.cnv <- tumor.seg.cnv[tumor.seg.cnv$Sample %in% rownames(subset(seg.kaps, kaps.group=="set3")),]
set4.seg.cnv <- tumor.seg.cnv[tumor.seg.cnv$Sample %in% rownames(subset(seg.kaps, kaps.group=="set4")),]
head(set1.seg.cnv)
write.table(set1.seg.cnv, file="4.model/kaps/mutation/cnv.v2/crc/tumor.set1.seg.txt", sep="\t", row.names=F, quote = F)
write.table(set2.seg.cnv, file="4.model/kaps/mutation/cnv.v2/crc/tumor.set2.seg.txt", sep="\t", row.names=F, quote = F)
write.table(set3.seg.cnv, file="4.model/kaps/mutation/cnv.v2/crc/tumor.set3.seg.txt", sep="\t", row.names=F, quote = F)
write.table(set4.seg.cnv, file="4.model/kaps/mutation/cnv.v2/crc/tumor.set4.seg.txt", sep="\t", row.names=F, quote = F)
...
pdf("4.model/kaps/mutation/cnv.v2/crc/cnv.scores.gistic.pdf",18,12)
par(mfrow=c(4,1), mar = par()$mar + c(3,0,0,3))


### set1 ###
scores <- read.table("4.model/kaps/mutation/cnv.v2/crc/set1.gisticout/scores.gistic", sep="\t",header=T,stringsAsFactors = F)
...
### set2 ###
scores <- read.table("4.model/kaps/mutation/cnv.v2/crc/set2.gisticout/scores.gistic", sep="\t",header=T,stringsAsFactors = F)
...
### set3 ###
scores <- read.table("4.model/kaps/mutation/cnv.v2/crc/set3.gisticout/scores.gistic", sep="\t",header=T,stringsAsFactors = F)
...
### set4 ###
scores <- read.table("4.model/kaps/mutation/cnv.v2/crc/set4.gisticout/scores.gistic", sep="\t",header=T,stringsAsFactors = F)
...
#########frequency plot
#############
pdf("4.model/kaps/mutation/cnv.v2/crc/cnv.frequence.pdf",18,12)
par(mfrow=c(4,1), mar = par()$mar + c(3,0,0,3))
