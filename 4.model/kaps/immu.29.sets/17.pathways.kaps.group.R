...
cbscoreshow$kaps.group=factor(cbscoreshow$kaps.group,levels = c("set4", "set3","set2","set1"))
index.sets=colnames(cbscoreshow)[c(2:30)]
for (i in 1:length(index.sets)) {
  pdf(file = paste0("4.model/kaps/immu.29.sets/indi/",index.sets[i],".immunesets.four.pdf"),height  = 4,width = 3)
  print(ggviolin(cbscoreshow,x = "kaps.group", y =index.sets[i] , fill = "kaps.group",alpha = 1,size = 0.3,
          ...
          ggtitle(paste0(index.sets[i],"CRC.immunesets"))
  )
  dev.off()
}
#17 sets
library(tidyr)
drawdatashow <- cbscoreshow %>% pivot_longer(cols=colnames(cbscoreshow)[2:19],
                                             names_to= "signature",
                                             values_to = "score")
...
generate.PDF <- function(fig) {
  pdf("4.model/kaps/immu.29.sets/17.pathways.pdf",height  = 8)
  print(p1)
  print(p2)
  dev.off()
}
...
#########
############
drawdatashow <- cbscoreshow %>% pivot_longer(cols=colnames(cbscoreshow)[20:30],
                                             names_to= "signature",
                                             values_to = "score")
drawdatashow=as.data.frame(drawdatashow)
...
#theme(strip.text.x = element_text(size = 8, colour = "orange", angle = 90))
generate.PDF <- function(fig) {
  pdf("4.model/kaps/immu.29.sets/11.immune.genes.pdf",height  = 8)
  print(p1)
  print(p2)
  dev.off()
}
